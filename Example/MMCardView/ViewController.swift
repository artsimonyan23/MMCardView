...
class ViewController: UIViewController {
    ...
    @IBAction func filterAction() {
        let sheet = UIAlertController(title: "Filter", message: "Select you want to show in View", preferredStyle: .actionSheet)
        ...
        let removeA = UIAlertAction(title: "Remove A", style: .default, handler: {
            ...
            self.cardCollection.deleteItems(at: [IndexPath(row: 0, section: 0)])
        })
        ...
        let removeB = UIAlertAction(title: "Remove B", style: .default, handler: {
            ...
            self.cardCollection.deleteItems(at: [IndexPath(row: 0, section: 1)])
        })
        ...
        let appendB = UIAlertAction(title: "Append B", style: .default, handler: {
            ...
            self.cardCollection.insertItems(at: [IndexPath(row: 0, section: 1)])
        })
        let appendC = UIAlertAction(title: "Append C", style: .default, handler: {
            (_: UIAlertAction!) -> Void in
            self.sectionData[2].insert("CardC", at: 0)
            self.cardCollection.insertItems(at: [IndexPath(row: 0, section: 2)])
        })
...
extension ViewController: UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        return card(collectionView: collectionView, cellForItemAt: indexPath)
    }
    ...
    fileprivate func card(collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case let c as CardCCell:
            ...
            c.clickCallBack {
                if let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "Second") as? SecondViewController {
                    vc.delegate = self
...
extension ViewController: SecondViewProtocol {
    func removeCard() {
        if let layout = cardCollection.collectionViewLayout as? CustomCardLayout, let path = layout.selectPath {
