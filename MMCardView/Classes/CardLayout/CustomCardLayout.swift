...
public class CustomCardLayout: UICollectionViewLayout {
    ...
    func updateCellSize() {
        ...
        cellSize.height = cardHeight * BottomPercent
    }
    ...
    fileprivate func setNoSelect(attribute: CardLayoutAttributes) {
        ...
        currentFrame = CGRect(x: collectionView!.frame.origin.x, y: titleHeight * CGFloat(index), width: cellSize.width, height: cellSize.height)
        switch showStyle {
        case .cover:

            if index <= shitIdx && (index >= shitIdx) {
                attribute.frame = CGRect(x: currentFrame.origin.x, y: collectionView!.contentOffset.y, width: cellSize.width, height: cellSize.height)
    ...
    fileprivate func setSelect(attribute: CardLayoutAttributes) {
        ...
        attribute.frame = CGRect(x: collectionView!.frame.origin.x, y: collectionView!.contentOffset.y + 0.01, width: cellSize.width, height: cellSize.height)
    }

    fileprivate func setBottom(attribute: CardLayoutAttributes, bottomIdx: inout CGFloat) {
        let baseHeight = collectionView!.contentOffset.y + collectionView!.bounds.height * 0.90
        let bottomH = cellSize.height * 0.1
        let margin: CGFloat = bottomH / CGFloat(bottomShowCount - 1)
        attribute.isExpand = false
        let yPos = isFullScreen ? (collectionView!.contentOffset.y + collectionView!.bounds.height) : bottomIdx * margin + baseHeight
